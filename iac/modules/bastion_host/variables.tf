...
variable "vm_net_subnet_cidr_second" {
  type        = string
  description = "Address prefix for the internal network (e.g., 192.168.1.0/24)"
}
# variable "vm_net_subnet_cidr_third" {
#   type        = string
#   description = "Address prefix for the internal network (e.g., 192.168.1.0/24)"
# }
variable "node_count" {
...
locals {
  ...
  vm_net_subnet_mask_second = "/${split("/", var.vm_net_subnet_cidr_second)[1]}"
  # First usable IP as the gateway
  vm_net_default_gw_second = cidrhost(var.vm_net_subnet_cidr_second, 1)
